...
// Define the currentADT pointer
ADTInterface* currentADT = nullptr; // Points to the currently selected ADT
...
int main() {
    ...
    std::vector<std::string> adtMenuOptions = {"Heap", "BST", "Red-Black Tree", "StackQueue", "AVL", "BTree", "PriorityQueue"};
    ...
    while (window.isOpen()) {
        ...
        // Call visualize for the current ADT
        if (currentADT) {
            currentADT->visualize(window, font);
        }
